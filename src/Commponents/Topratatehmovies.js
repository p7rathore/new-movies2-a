...
const PopularHmovies = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/top_rated?api_key=f25a85c11354cad69a2a3d1b9bc9e6b9&language=en-US&page=1`
    )
      .then((res) => res.json())
      .then((populardata) => {
  ...
  let crousel1 = document.getElementById("crousel1");

  const preview = () => {
    crousel1.scrollLeft -= 250;
  };
  const next = () => {
    crousel1.scrollLeft += 250;
  };
  ...
  return (
    <>
      <div className="popular-movies">
        <h1 className="popular-movie-text">Top_rated Movies</h1>
        <div className="movies-po">
          <div className="card" id="crousel1">
