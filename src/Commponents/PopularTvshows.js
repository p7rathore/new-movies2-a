...
const PopularHmovies = () => {
  ...
  useEffect(() => {
    fetch(
      `https://api.themoviedb.org/3/movie/upcoming?api_key=f25a85c11354cad69a2a3d1b9bc9e6b9&language=en-US&page=1`
    )
      .then((res) => res.json())
      .then((populardata) => {
  ...
  let crouse2 = document.getElementById("crouse2");

  const preview = () => {
    crouse2.scrollLeft -= 250;
  };
  const next = () => {
    crouse2.scrollLeft += 250;
  };
  ...
  return (
    <>
      <div className="popular-movies">
        <h1 className="popular-movie-text"> upcomming movies</h1>
        <div className="movies-po">
          <div className="card" id="crouse2">
                ...
                <>
                  ...
                  >
                    <img
                      ...
                      alt="card.jpg"
                    />
                    <h2 className="card-movies-name">{res.original_name}</h2>
                  </div>
                </>
              );
            })}
